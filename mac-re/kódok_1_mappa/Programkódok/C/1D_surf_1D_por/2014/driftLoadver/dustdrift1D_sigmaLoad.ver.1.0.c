...
/*	A porreszecskek adatainak beolvasasa	*/
void sigma_be(double radius[][2], double *mass) {
...
void Initial_Profile(double SIGMA0, double *sigmavec, double *r, double DD, double RMIN, double RMAX){		/*	initial profile of sigma		*/

  	int i;
	double sig0 = SIGMA0 / pow(RMIN,SIGMAP_EXP);
	printf("sig0 r=1AU-n: %lg\n",sig0);
  
  	for(i = 1; i <= NGRID; i++) {
    		sigmavec[i] = sig0 * pow(r[i],SIGMAP_EXP);		/*	sigma0*r^x (x could be eg. -1/2)	*/
  	}
...
/*	Kezdeti profilok betoltese a megadott vektorokba	*/
	Initial_Profile(SIGMA0,sigmavec,rvec,DD,RMIN,RMAX);
...
/*	ez utan kiszamolhato az eredeti t_drift keplet, ezt iratom ki a file-ba evben (ezert osztok le 2PI-vel	*/
		...
		if(max >= RMIN) {							/*	Ha a legtavolabbi porreszke merete nagyobb RMIN-nel, akkor folytatodik az integralas, kulonben kilep	*/
			...
			uvec[0] = sigmavec[0] * visc(rvec[0],alpha_visc);
			uvec[NGRID+1] = sigmavec[NGRID+1] * visc(rvec[NGRID+1],alpha_visc);

  			for(i = 1; i <= NGRID; i++) {					/* 	solving d(sigma*nu)/dt = 3*nu*d2(sigma*nu)/dr2 + 9*nu/(2*r)*dsigma/dr	*/
				u = sigmavec[i] * visc(rvec[i],alpha_visc);
				u_bi = sigmavec[i-1] * visc(rvec[i-1],alpha_visc);
				u_fi = sigmavec[i+1] * visc(rvec[i+1],alpha_visc);
				uvec[i] = u;
				double temp = Coeff_1(rvec[i],alpha_visc) * (u_fi - 2.0 * u + u_bi) / (DD * DD) + Coeff_2(rvec[i],alpha_visc) * (u_fi - u_bi) / (2.0 * DD);
			...
			for(i = 1; i <= NGRID; i++) {
				sigmavec[i] = sigma_temp[i]/visc(rvec[i],alpha_visc);
				pressvec[i] = press(sigmavec[i],rvec[i]);
			}
