...
void u_gas(double *sigmavec, double *rvec, double *ug) {
	...
	for(i = 1; i <= NGRID; i++) {
		...
		ug[i] = Coeff_3(sigmavec[i],rvec[i]) * tempug;
	}
...
void GetMass(int n, double partmassind[][4],int indii, int indio, double tav1, double dzetav1, double *massiout,int indoi, int indoo, double tav2, double dzetav2, double *massoout, int opt) {
...
/* 	MEG KELL OLDANI, HOGY AKKOR IS TUDJON TOMEGNOVEKEDEST SZAMOLNI, HA CSAK BELSO DZE VAN!	*/
	...
	double masst0i = 0, massii = 0, masst0o = 0, massoi = 0;
	double masst0im = 0, massiim = 0, masst0om = 0, massoim = 0;
	double massis = 0, massos = 0;


	GetMass(PARTICLE_NUMBER,partmassind,(int)ind_ii,(int)ind_io,tav2,r_dze_i,&massii,(int)ind_oi,(int)ind_oo,tav,r_dze_o,&massoi,0);
	
	if(opttwopop == 1) {
		GetMass(4*PARTICLE_NUMBER,partmasssecind,(int)ind_ii,(int)ind_io,tav2,r_dze_i,&massis,(int)ind_oi,(int)ind_oo,tav,r_dze_o,&massos,1);
		GetMass(PARTICLE_NUMBER,partmassmicrind,(int)ind_ii,(int)ind_io,tav2,r_dze_i,&massiim,(int)ind_oi,(int)ind_oo,tav,r_dze_o,&massoim,0);
	}
...
void Print_Sigmad(char *dust_name, char *dust_name2, double min, double *r, double *rm, double *sigmad, double *sigmadm) {
	...
	switch((int)opttwopop) {
		...
		case 1:
			sid = fopen(dust_name2,"w");		/*	Ha 2pop --> megnyit egy kulon file-t a mikronos por feluletisurusegenek kiiratasara	*/
...
/*	ha van reszecske novekedes, akkor letrehoz egy file-t minden adott idolepesben, ami a centimeteres porreszecskek meretet tartalmazza	*/
		...
		case 1: 
			...
			fout3 = fopen(size_name,"w");	/*	ha van pornovekedes	*/
...
void loadSigDust(double radin[][2], double *massin, double out[][3],double radmin[][2], double *massmin, double outm[][3],double radsin[][2], double *masssin, double outs[][3], double dd) {
	...
	switch((int)opttwopop) {
		...
		case 0:
/*	Ha nem 2 populacios a szimulacio	*/
...
/*	mikronos por feluletisurusegenek kiszamolasa	*/
/*	ha a reszecske tavolsaga nagyobb, mint RMIN, azaz a szamolas tartomanyan belul van, a feluletisuruseget az altala kepviselt tomegbol szamolja vissza a program	*/
				if((radmin[i][0] >= RMIN)) {
					outm[i][0] = massmin[i] / (2. * (radmin[i][0]-dd/2.) * M_PI * dd);	// sigma = m /(2 * r * pi * dr) --> itt a dr az a tavolsag, ami porreszecske "generalo" programban az eredeti gridfelbontas
					outm[i][1] = radmin[i][0];					// elmenti a reszecske tavolsagat is

  					double rmid = (radmin[i][0] - RMIN) / dd;     						/* 	The integer part of this gives at which index is the body			*/
					int rindex = (int) floor(rmid);							/* 	Ez az rmid egesz resze --> floor egeszreszre kerekit lefele, a +0.5-el elerheto, hogy .5 felett felfele, .5 alatt lefele kerekitsen						*/
					outm[i][2] = (double) rindex;

/*	ha a reszecske RMIN-en belul van, akkor az o "tavolsagaban" a feluletisuruseg 0		*/	
				} else {
					outm[i][0] = 0;
					outm[i][1] = 0;					// r = 0, mert ha RMIN-en belulre kerult a reszecske, akkor a program automatikusan kinullazza a reszecske tavolsagat. Itt tehat a sigdtemp[i][1] = 0 lesz!
					outm[i][2] = 0;
				}


/*	a novesztett por feluletisurusegenek kiszamolasa	*/
/*	ha a reszecske tavolsaga nagyobb, mint RMIN, azaz a szamolas tartomanyan belul van, a feluletisuruseget az altala kepviselt tomegbol szamolja vissza a program	*/
				if((radsin[i][0] >= RMIN)) {
					outs[i][0] = masssin[i] / (2. * (radsin[i][0]-dd/2.) * M_PI * dd);	// sigma = m /(2 * r * pi * dr) --> itt a dr az a tavolsag, ami porreszecske "generalo" programban az eredeti gridfelbontas
					outs[i][1] = radsin[i][0];					// elmenti a reszecske tavolsagat is

  					double rmid = (radsin[i][0] - RMIN) / dd;     						/* 	The integer part of this gives at which index is the body			*/
					int rindex = (int) floor(rmid);							/* 	Ez az rmid egesz resze --> floor egeszreszre kerekit lefele, a +0.5-el elerheto, hogy .5 felett felfele, .5 alatt lefele kerekitsen						*/
					outs[i][2] = (double) rindex;

/*	ha a reszecske RMIN-en belul van, akkor az o "tavolsagaban" a feluletisuruseg 0		*/	
				} else {
					outs[i][0] = 0;
					outs[i][1] = 0;					// r = 0, mert ha RMIN-en belulre kerult a reszecske, akkor a program automatikusan kinullazza a reszecske tavolsagat. Itt tehat a sigdtemp[i][1] = 0 lesz!
					outs[i][2] = 0;
				}
				if((radsin[i+PARTICLE_NUMBER][0] >= RMIN)) {
					outs[i+PARTICLE_NUMBER][0] = masssin[i+PARTICLE_NUMBER] / (2. * (radsin[i+PARTICLE_NUMBER][0]-dd/2.) * M_PI * dd);	// sigma = m /(2 * r * pi * dr) --> itt a dr az a tavolsag, ami porreszecske "generalo" programban az eredeti gridfelbontas
					outs[i+PARTICLE_NUMBER][1] = radsin[i+PARTICLE_NUMBER][0];					// elmenti a reszecske tavolsagat is

  					double rmid = (radsin[i+PARTICLE_NUMBER][0] - RMIN) / dd;     						/* 	The integer part of this gives at which index is the body			*/
					int rindex = (int) floor(rmid);							/* 	Ez az rmid egesz resze --> floor egeszreszre kerekit lefele, a +0.5-el elerheto, hogy .5 felett felfele, .5 alatt lefele kerekitsen						*/
					outs[i+PARTICLE_NUMBER][2] = (double) rindex;

/*	ha a reszecske RMIN-en belul van, akkor az o "tavolsagaban" a feluletisuruseg 0		*/	
				} else {
					outs[i+PARTICLE_NUMBER][0] = 0;
					outs[i+PARTICLE_NUMBER][1] = 0;					// r = 0, mert ha RMIN-en belulre kerult a reszecske, akkor a program automatikusan kinullazza a reszecske tavolsagat. Itt tehat a sigdtemp[i][1] = 0 lesz!
					outs[i+PARTICLE_NUMBER][2] = 0;
				}

				if((radsin[i+2*PARTICLE_NUMBER][0] >= RMIN)) {
					outs[i+2*PARTICLE_NUMBER][0] = masssin[i+2*PARTICLE_NUMBER] / (2. * (radsin[i+2*PARTICLE_NUMBER][0]-dd/2.) * M_PI * dd);	// sigma = m /(2 * r * pi * dr) --> itt a dr az a tavolsag, ami porreszecske "generalo" programban az eredeti gridfelbontas
					outs[i+2*PARTICLE_NUMBER][1] = radsin[i+2*PARTICLE_NUMBER][0];					// elmenti a reszecske tavolsagat is

  					double rmid = (radsin[i+2*PARTICLE_NUMBER][0] - RMIN) / dd;     						/* 	The integer part of this gives at which index is the body			*/
					int rindex = (int) floor(rmid);							/* 	Ez az rmid egesz resze --> floor egeszreszre kerekit lefele, a +0.5-el elerheto, hogy .5 felett felfele, .5 alatt lefele kerekitsen						*/
					outs[i+2*PARTICLE_NUMBER][2] = (double) rindex;

/*	ha a reszecske RMIN-en belul van, akkor az o "tavolsagaban" a feluletisuruseg 0		*/	
				} else {
					outs[i+2*PARTICLE_NUMBER][0] = 0;
					outs[i+2*PARTICLE_NUMBER][1] = 0;					// r = 0, mert ha RMIN-en belulre kerult a reszecske, akkor a program automatikusan kinullazza a reszecske tavolsagat. Itt tehat a sigdtemp[i][1] = 0 lesz!
					outs[i+2*PARTICLE_NUMBER][2] = 0;
				}

				if((radsin[i+3*PARTICLE_NUMBER][0] >= RMIN)) {
					outs[i+3*PARTICLE_NUMBER][0] = masssin[i+3*PARTICLE_NUMBER] / (2. * (radsin[i+3*PARTICLE_NUMBER][0]-dd/2.) * M_PI * dd);	// sigma = m /(2 * r * pi * dr) --> itt a dr az a tavolsag, ami porreszecske "generalo" programban az eredeti gridfelbontas
					outs[i+3*PARTICLE_NUMBER][1] = radsin[i+3*PARTICLE_NUMBER][0];					// elmenti a reszecske tavolsagat is

  					double rmid = (radsin[i+3*PARTICLE_NUMBER][0] - RMIN) / dd;     						/* 	The integer part of this gives at which index is the body			*/
					int rindex = (int) floor(rmid);							/* 	Ez az rmid egesz resze --> floor egeszreszre kerekit lefele, a +0.5-el elerheto, hogy .5 felett felfele, .5 alatt lefele kerekitsen						*/
					outs[i+3*PARTICLE_NUMBER][2] = (double) rindex;

/*	ha a reszecske RMIN-en belul van, akkor az o "tavolsagaban" a feluletisuruseg 0		*/	
				} else {
					outs[i+3*PARTICLE_NUMBER][0] = 0;
					outs[i+3*PARTICLE_NUMBER][1] = 0;					// r = 0, mert ha RMIN-en belulre kerult a reszecske, akkor a program automatikusan kinullazza a reszecske tavolsagat. Itt tehat a sigdtemp[i][1] = 0 lesz!
					outs[i+3*PARTICLE_NUMBER][2] = 0;
				}
			}
...
void Get_Sigmad(double L, double max, double min, double rad[][2], double radmicr[][2], double radsec[][2], double *sigma_d, double *sigma_dm, double *sigma_ds, double *massvec, double *massmicrvec, double *masssecvec, double *rd, double *rmic, double *rs) {
	...
	for(i=0;i<PARTICLE_NUMBER;i++){
		...
		radsectemp[i] = 0;
		radsectemp[i+PARTICLE_NUMBER] = 0;
		radsectemp[i+2*PARTICLE_NUMBER] = 0;
		radsectemp[i+3*PARTICLE_NUMBER] = 0;
		sdsectemp[i] = 0;
		sdsectemp[i+PARTICLE_NUMBER] = 0;
		sdsectemp[i+2*PARTICLE_NUMBER] = 0;
		sdsectemp[i+3*PARTICLE_NUMBER] = 0;
		sigdsectemp[i][0] = 0;
		sigdsectemp[i+PARTICLE_NUMBER][0] = 0;
		sigdsectemp[i+2*PARTICLE_NUMBER][0] = 0;
		sigdsectemp[i+3*PARTICLE_NUMBER][0] = 0;
		sigdsectemp[i][1] = 0;
		sigdsectemp[i+PARTICLE_NUMBER][1] = 0;
		sigdsectemp[i+2*PARTICLE_NUMBER][1] = 0;
		sigdsectemp[i+3*PARTICLE_NUMBER][1] = 0;
		sigdsectemp[i][2] = 0;
		sigdsectemp[i+PARTICLE_NUMBER][2] = 0;
		sigdsectemp[i+2*PARTICLE_NUMBER][2] = 0;
		sigdsectemp[i+3*PARTICLE_NUMBER][2] = 0;
		rs[i] = 0;
		rs[i+PARTICLE_NUMBER] = 0;
		rs[i+2*PARTICLE_NUMBER] = 0;
		rs[i+3*PARTICLE_NUMBER] = 0;

	}

	loadSigDust(rad,massvec,sigdtemp,radmicr,massmicrvec,sigdmicrtemp,radsec,masssecvec,sigdsectemp,dd);
...
/*	Az elozohoz hasonloan interpolalja a mikronos feluletisuruseget a cm-es porszemcsek tavolsagara		*/	
				if((int)sigdmicrtemp[j][2] >= rtempi[i] && (int)sigdmicrtemp[j][2] < rtempi[i+1]){
					...
					double rd = rtemp[i+1]-rtemp[i];
					interpol(sigtempvec,rtemp,y,&simt2,dd,opttwopop);
					r2 = sigdmicrtemp[j][1];
					l++;
				} 
			...
			if(simt2 != 0) {
				sigma_dm[l] = simt2;	/*	ide rakja az interpolalt feluletisuruseget	*/
			}
		}
...
/*	A porszemcse generalo program gridfelbontasa -- nem feltetlen egyezik meg a jelenlegi gridfelbontassal!	*/
	...
	for(i=0; i < PARTICLE_NUMBER; i++) {
		...
		temp2[i+PARTICLE_NUMBER] = radsec[i+PARTICLE_NUMBER][0];			// masodlagosan novesztett porszemcsek tavolsaga
		temp2[i+2*PARTICLE_NUMBER] = radsec[i+2*PARTICLE_NUMBER][0];			// masodlagosan novesztett porszemcsek tavolsaga
		temp2[i+3*PARTICLE_NUMBER] = radsec[i+3*PARTICLE_NUMBER][0];			// masodlagosan novesztett porszemcsek tavolsaga
		sizetemp[i] = radsec[i][1];
		sizetemp[i+PARTICLE_NUMBER] = radsec[i+PARTICLE_NUMBER][1];
		sizetemp[i+2*PARTICLE_NUMBER] = radsec[i+2*PARTICLE_NUMBER][1];
		sizetemp[i+3*PARTICLE_NUMBER] = radsec[i+3*PARTICLE_NUMBER][1];
		temptemp[i] = 0;				// atmeneti vektor, amely elmeni, hogy hol noveszt uj reszecsket a program
		temptemp[i+PARTICLE_NUMBER] = 0;				// atmeneti vektor, amely elmeni, hogy hol noveszt uj reszecsket a program
		temptemp[i+2*PARTICLE_NUMBER] = 0;				// atmeneti vektor, amely elmeni, hogy hol noveszt uj reszecsket a program
		temptemp[i+3*PARTICLE_NUMBER] = 0;				// atmeneti vektor, amely elmeni, hogy hol noveszt uj reszecsket a program
		masstemp[i][0] = 0;				// atemeneti tomb, amely a novesztett reszecske tomeget tarolja el
		masstemp[i+PARTICLE_NUMBER][0] = 0;				// atemeneti tomb, amely a novesztett reszecske tomeget tarolja el
		masstemp[i+2*PARTICLE_NUMBER][0] = 0;				// atemeneti tomb, amely a novesztett reszecske tomeget tarolja el
		masstemp[i+3*PARTICLE_NUMBER][0] = 0;				// atemeneti tomb, amely a novesztett reszecske tomeget tarolja el
		masstemp[i][1] = 0;				// atmeneti tomb, amely a novesztett reszecske tavolsagat tarolja el
		masstemp[i+PARTICLE_NUMBER][1] = 0;
		masstemp[i+2*PARTICLE_NUMBER][1] = 0;
		masstemp[i+3*PARTICLE_NUMBER][1] = 0;
		mtp[i] = masssecvec[i];
		mtp[i+PARTICLE_NUMBER] = masssecvec[i+PARTICLE_NUMBER];
		mtp[i+2*PARTICLE_NUMBER] = masssecvec[i+2*PARTICLE_NUMBER];
		mtp[i+3*PARTICLE_NUMBER] = masssecvec[i+3*PARTICLE_NUMBER];
		indtemp[i][0] = partsecind[i][3];
		indtemp[i+PARTICLE_NUMBER][0] = partsecind[i+PARTICLE_NUMBER][3];
		indtemp[i+2*PARTICLE_NUMBER][0] = partsecind[i+3*PARTICLE_NUMBER][3];
		indtemp[i+3*PARTICLE_NUMBER][0] = partsecind[i+2*PARTICLE_NUMBER][3];
...
/*	Valamiert csak ugy ad jo kepet, ha az if-es felteleket beirom.	*/
		...
		if(temp2[i+PARTICLE_NUMBER] != 0) step++;			// megszamolja, hogy hany novesztett reszecske van a fuggvenybe valo belepeskor
		if(temp2[i+2*PARTICLE_NUMBER] != 0) step++;			// megszamolja, hogy hany novesztett reszecske van a fuggvenybe valo belepeskor
		if(temp2[i+3*PARTICLE_NUMBER] != 0) step++;			// megszamolja, hogy hany novesztett reszecske van a fuggvenybe valo belepeskor
...
/*	Ez az az eset, ha a program nem szamol driftet, azaz csak a gaz feluletisurusegenek fejlodesere vagyunk kivancsiak	*/
...
/******************************************************************************************************************************/
/*	Ez az az eset, ha a program szamol driftet	*/

		case 1:

			switch((int)opttwopop) {

/*	Ez az az eset, ha a van drift es nem 2 populacios a modell	*/
...
/*	Ha van pornovekedes, kiirja a por felultisuruseget egy file-ba --> a pornovekedeshez szukseges egyaltalan ezt kiszamolni!	*/
								if(optgr == 1.) {
									Print_Sigmad(dust_name,dust_name2,min,rdvec,rmicvec,sigmad,sigmadm);
								}
...
/*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/
/*	Ez az az eset, ha a van drift es 2 populacios a modell		*/

				case 1:
	
			  	 	do {

						secondaryGrowth(radius,radiusmicr,radiussec,partmassmicrind,partmasssecind,massvec,massmicrvec,masssecvec, L);
/*	A minimum kereseshez letrehozza a cm-es reszecskek tavolsaganak reciprokat	*/
						for (i=0; i < PARTICLE_NUMBER; i++) {
							if (radius[i][0] > 0. && radius[i][0] > RMIN) {
								radius_rec[i][0] = 1. / radius[i][0];
...
/*	A reszecskek tomeget tartalmazo tomb inicializalasa		*/
									Count_Mass(radius,partmassind,massvec,t,PARTICLE_NUMBER);
									Count_Mass(radiusmicr,partmassmicrind,massmicrvec,t,PARTICLE_NUMBER);
									Count_Mass(radiussec,partmasssecind,masssecvec,t,4*PARTICLE_NUMBER);
