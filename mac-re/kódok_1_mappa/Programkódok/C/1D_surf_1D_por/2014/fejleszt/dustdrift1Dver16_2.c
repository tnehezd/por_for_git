...
double RMINDUST, RMAXDUST;				/*	korong kulso es belso hatara				*/
...
double optev, optdr, optgr, opttwopop, optdze, optinp, optrmin, optrmax;	/*	parameterek, amelyekkel ki es be lehet kapcsolni a szamolasok egy reszet:
...
double fFrag, uFrag, fDrift;					/*	a reszecske novekedeshez szukseges parameterek Birnstiel et al 2012 alapjan:
									fFrag: ?
...
int parse_options(int argc, const char **argv, options_t *opt){
	...
	while (i < argc) {
		...
		else if (strcmp(p, "-ufrag") == 0) {
			i++;
			opt->ufrag = atof(argv[i]);
		}
		else if (strcmp(p, "-ffrag") == 0) {
			i++;
			opt->ffrag = atof(argv[i]);
		}
		else if (strcmp(p, "-udrift") == 0) {
			i++;
			opt->fdrift = atof(argv[i]);
		}
...
//			getchar();
	...
	fclose(fin1);
	RMINDUST = radius[0][0];
	RMAXDUST = radius[0][PARTICLE_NUMBER-1];	
...
void Print_Sigmad(char *dust_name, char *dust_name2, double min, double *r, double *rm, double *sigmad, double *sigmadm) {
	...
	double dd = (RMAXDUST - RMINDUST) / (PARTICLE_NUMBER-1);	/*	Mivel a jelen futas gridfelbontasa nem feltetlen egyezik meg a porreszecskeket generalo program gridfelbontasaval - ez a feluletisuruseg miatt lenyeges! - ezert itt szamolja ki a program	*/
...
/*	A porszemcse generalo program gridfelbontasa -- nem feltetlen egyezik meg a jelenlegi gridfelbontassal!	*/
	double dd = (RMAXDUST - RMINDUST) / (PARTICLE_NUMBER-1), rtempvec[PARTICLE_NUMBER];
...
/*	A sigmat tartalmazo file sorainak szama elmentve egy integerbe	*/
	...
	char dens_name[1024], nev[10024], mv[1024];
...
/*	A korong parametereinek beolvassa				*/
	...
	if(optrmin == 1.) RMIN = def.Rmin;
	if(optrmax == 1.) RMAX = def.Rmax;
