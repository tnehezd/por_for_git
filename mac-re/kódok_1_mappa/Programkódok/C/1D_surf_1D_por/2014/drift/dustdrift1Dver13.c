...
double tauGr(double r) {

	double eps = 100.;			/*	sigma gas to sigma dust ratio	*/
...
double getSize(double prad, double pdens, double sigma, double y, double p, double dpress, double dt) {
...
/*		sturb = a_turb(sigma,y,pdens);
		...
		prad = smin / AU2CM;
*/
		pradtemp = getSize(prad,pdens,sigma,y,p,dpress,step);
		prad = pradtemp;
...
void Get_Radius(char *nev, double radius[][2], double *pressvec, double *dpressvec, double *sigmavec, double *rvec, double *ugvec, double deltat, double t) {		/*	a bemenet dimenziótlan egységekben van!!!	*/
...
/*	A resezcskek adatait tartalmazo file sorainak szama (azaz a reszecskek szama) elmentve egy integerbe	*/
	...
	char mv[1024], dens_name[1024],size_name[1024], porout[1024], massout[1024];
...
/*	Ha az optev erteke 1, akkor megoldja minden lepesben a sigmara vonatkozo diffuzios egyenletet	*/
				if(optev == 1.) Get_Sigma_P_dP(rvec, sigmavec, pressvec, dpressvec, deltat);	
