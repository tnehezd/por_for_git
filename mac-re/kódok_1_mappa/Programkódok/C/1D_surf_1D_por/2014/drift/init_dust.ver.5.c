...
int parse_options(int argc, const char **argv, options_t *opt, double *par, double *dpar, double *exppar){
	int i = 1;
	...
	while (i < argc) {
		...
		if(strcmp(p, "-n") == 0) {
			...
			printf("**** Try following parameters: ****\n\n-n: number of grids (particles)\n-ri: inner edge of disk\n-ro: other edge of disk\n-sigma0: sigma at r=1AU\n-sigma0cgs: sigma at r=1AU in cgs\n-index: sigma exponent\n-rdzei: inner DZE\n-rdzeo: other DZE\n-drdzei: the width (in H, eg. 2H, 1.5H, etc) of the viscosity jump at the inner deadzone edge (if not given, but rdzei is given, set to 2*H)\n-drdzeo: the width (in H, eg. 2H, 1.5H, etc) of the viscosity jump at the outer deadzone edge (if not given, but rdzeo is given, set to 2*H)\n-alpha: alpha viscosity\n-amod: viscosity reduction\n-h: aspect ratio\n-flind:flaring index\n-m0: mass of the central star (in solar masses)\n-md: mass of the disk\n-eps: dust to gas ratio\n\n\n");
			return 1;
		}
		i++;
		free(p);
...
int main(int argc, const char **argv) {  
	...
	double par, par2, par3;
	int retCode = parse_options(argc, argv, &def, &par, &par2, &par3);
	...
	double v_kep, c_s, v_kep2, c_s2, Sigma, Sigma_cgs, Sigmad_cgs, P, dPdr, dlnPdlnr, rho_p, rho_mp, s_drift, s_frag, s_df;
  	double s_max;
...
//		printf("dust: %lg, dust dr: %lg\n", sigma_dust(reval), 2.0 * M_PI * r * DD * sigma_dust(reval));
