...
void find_r_annulus(double *rvec, double rout, double *ind_oi, double *ind_oo) {
	...
	int i;
	double rmid, rtemp;
	double roimH = (rout - scale_height(rout)) - DD / 2.0;		/*	A nyomasi maximum az rout pontban van, ettol rout - H - DD / 2 es rout + H -DD / 2 kozott van a korgyuru belso hatara (azert DD/2, hogy biztosan 1 cellat tudjunk kijelolni, ne pedig egy tartomanyt)	*/
...
void Print_Mass(double step, double *rvec, double partmassind[][4], double partmassmicrind[][4], double t, double *dpressvec) {
...
/* 	MEG KELL OLDANI, HOGY AKKOR IS TUDJON TOMEGNOVEKEDEST SZAMOLNI, HA CSAK BELSO DZE VAN!	*/
	...
	find_r_annulus(rvec,tav2,&ind_ii,&ind_io);		/*	A beslo nyomasi maximum korul 2H tavolsagban keres korgyurut, a fuggveny visszaadja a cellak indexet	*/

	int index_i,index_i2,i;
	double mass = 0., massi = 0., massi2 = 0., mass2 = 0.;
...
/*	Ha a reszecse a kiszamolt korgyuruben tartozkodik, akkor a mass valtozohoz hozzaadjuk a tomeget --> igy tudjuk a tomegnovekedest kiszamolni	*/
		if ((index_i >= (int)ind_oi) && (index_i <= (int)ind_oo)) {
			...
			masstempo = partmassind[i][0];		/*	Az adott reszecske altal kepviselt reprezentativ tomeg	*/	
			mass = mass + masstempo;
		}

/*	Ha a reszecse a kiszamolt korgyuruben tartozkodik, akkor a mass valtozohoz hozzaadjuk a tomeget --> igy tudjuk a tomegnovekedest kiszamolni	*/
		if ((index_i >= (int)ind_ii) && (index_i <= (int)ind_io)) {

			masstempi = partmassind[i][0];		/*	Az adott reszecske altal kepviselt reprezentativ tomeg	*/	
			massi = massi + masstempi;

		}

/*	A mikronos reszecskek tomegnovekedese	*/
/*	Hasonlo a fenti, cm-es porszemcsek altali tomegnovekedes meresehez	*/		
		if(opttwopop == 1.) {	/*	Ezt csak akkor szamolja, ha 2 populacios szimulacio fut eppen	*/
			index_i2 = partmassmicrind[i][1];
			double masstemp2 = 0.;
			double masstempo2 = 0.;

			if ((index_i2 >= (int)ind_ii) && (index_i2 <= (int)ind_io)) {
	
				masstempo2 = partmassmicrind[i][0];
				massi2 = massi2 + masstempo2;

			}

			if ((index_i2 >= (int)ind_oi) && (index_i2 <= (int)ind_oo)) {
	
				masstemp2 = partmassmicrind[i][0];
	...
	fprintf(massfil,"%lg %lg %lg %lg %lg\n",step,tav2,massi+massi2,tav,mass+mass2);
	fflush(massfil);
...
void Count_Mass(double radin[][2], double partmassindin[][4], double *massvecin, double t, int n) {
	...
	for (i = 0; i < n; i++) {	
		...
		partmassindin[i][0] = massvecin[i];							/*	mass of the particles				*/
...
void secondaryGrowth(double rad[][2], double radmicr[][2], double radsec[][2], double partmicind[][4], double partsecind[][4], double *massvec, double *massmicvec, double *masssecvec) {
...
/*	A porszemcse generalo program gridfelbontasa -- nem feltetlen egyezik meg a jelenlegi gridfelbontassal!	*/
	...
	double temptemp[4*PARTICLE_NUMBER], masstemp[4*PARTICLE_NUMBER][2], sizetemp[4*PARTICLE_NUMBER],mtp[4*PARTICLE_NUMBER];
...
/*	Ha van drift:	*/	
		if(optdr == 1.) {
			...
			if(opttwopop == 1) secondaryGrowth(radius,radiusmicr,radiussec,partmassmicrind,partmasssecind,massvec,massmicrvec,masssecvec);
