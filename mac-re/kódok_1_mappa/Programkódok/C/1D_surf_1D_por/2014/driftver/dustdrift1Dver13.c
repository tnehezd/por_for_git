...
void histogram(double r, int *hist) {
...
double getSize(double prad, double pdens, double sigma, double sigmad, double y, double p, double dpress, double dt) {
	...
	double smin = find_min(sturb,sdf,HUGE_VAL);		// cm-ben
...
void int_step(double time, double prad, double *pressvec, double *dpressvec, double *sigmavec, double sigmad, double *rvec, double *ugvec, double step, double y, double *ynew, double *pradnew) {
	...
	if(time != 0.) {
		...
		pdens = PDENSITY; //DIMLESS * SUN2GR / AU2CM / AU2CM / AU2CM; --> gr/cm^3!!!!!!!!!!!!!!
/*		sturb = a_turb(sigma,y,pdens);
		sdf = a_df(sigma,y,p,dpress,pdens);
		smin = find_min(sturb,sdf,HUGE_VAL);
		prad = smin / AU2CM;
*/
		pradtemp = getSize(prad,pdens,sigma,sigmad,y,p,dpress,step);
		prad = pradtemp;
...
void Print_Sigmad(char *dust_name, double radius[][2], double *sigmad) {
	...
	for(i=0;i<PARTICLE_NUMBER;i++){
		...
		if (radius[i][0] >= RMIN) fprintf(fil,"%lg  %lg \n",radius[i][0], sigmad[i]);
	}
...
void Print_Pormozg_Size_Hist(char *size_name, char *hist_name, int step, double rad[][2], double *rvec, int *hist, double t){
...
void Get_Sigmad(double rad[][2], double *massvec, double *sigma_d) {
	...
	for(i=0;i<PARTICLE_NUMBER;i++){
		...
		if(rad[i][0] > RMIN) {
			sigma_d[i] = massvec[i] / (2. * rad[i][0] * M_PI * dd);
		} else {
			sigma_d[i] = 0;
		}
...
void Get_Radius(char *nev, double radius[][2], double *pressvec, double *dpressvec, double *sigmavec, double *sigmad, double *rvec, double *ugvec, double deltat, double t, int *NUM) {		/*	a bemenet dimenziótlan egységekben van!!!	*/
...
void Count_Mass(double radius[][2], double partmassind[][3], double *massvec) {
...
/*	A resezcskek adatait tartalmazo file sorainak szama (azaz a reszecskek szama) elmentve egy integerbe	*/
	...
	char mv[1024], dens_name[1024],size_name[1024], porout[1024], massout[1024], hist_name[1024], dust_name[1024];
...
/*	A reszecskek adatainak beolvasasa file-bol, ha az optdr erteke 1, egyebkent nem szamol a program driftet	*/
	if(optdr == 1.) {
		...
		int dummy;
		sprintf(mv,"cp %s %s/",filenev1,nev);
		dummy = system(mv);	
...
/*	A minimum kereseshez letrehozza a reszecskek tavolsaganak reciprokat	*/
		if(optdr == 1.) {
			for (i=0; i < PARTICLE_NUMBER; i++) {
			...
			if(max >= RMIN && min != max) {						/*	Ha a legtavolabbi porreszke merete nagyobb RMIN-nel, akkor folytatodik az integralas, kulonben kilep	*/
				...
				if((fmod(time, (TMAX/WO)) < deltat || time == 0) && L-time < deltat){	
					...
					snprintf(hist_name,1024,"%s/hist.%i.dat",nev,(int)L);
					snprintf(dust_name,1024,"%s/dust.%i.dat",nev,(int)L);
					snprintf(size_name,1024,"%s/size.%d.dat",nev,(int)L);

					for(i = 0; i < PARTICLE_NUMBER; i++) histogram(radius[i][0],hist);		
	
					if(t==0) {
						Count_Mass(radius,partmassind,massvec);
						Get_Sigmad(radius, massvec, sigmad);
					}
					Print_Sigma(dens_name, rvec, sigmavec, pressvec, dpressvec);
					Print_Pormozg_Size_Hist(size_name,hist_name,(int)L,radius,rvec,hist,t);
					Print_Mass(deltat,rvec,partmassind,t,dpressvec);
					Print_Sigmad(dust_name,radius,sigmad);
...
/*	Ha az optev erteke 1, akkor megoldja minden lepesben a sigmara vonatkozo diffuzios egyenletet	*/
				...
				Get_Radius(nev,radius,pressvec,dpressvec,sigmavec,sigmad,rvec,ugvec,deltat,t,&num); 
