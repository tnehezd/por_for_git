...
*/
#define STEPWRITE	3000				/*	hanyadik lepesenkent irassa ki az adatokat	*/
...
void disk_param_be(double *sigma0, double *sdexp, double *Rmin, double *Rmax, double *r_dzei, double *r_dzeo, double *dr_dzei, double *dr_dzeo, double *alph_mod) {

	double dummy, rmin, rmax, drdzei, drdzeo, rdzei, rdzeo, amod;
	int dummy2;
	...
	fin2 = fopen("disk_param.dat","r");
           	...
           	if(fscanf(fin2,"%lg  %lg %d %lg %lg %lg %lg %lg %lg %lg %lg",&rmin,&rmax,&dummy2,&exp,&sig0,&dummy,&rdzei,&rdzeo,&drdzei,&drdzeo,&amod) == 11) { /*	A beolvasás sikeres, ha az fscanf visszatérési értéke 5, mert 5 oszlopot szeretnénk beolvasni.	*/
...
void Perem(double *sigmavec, double DD) {					/*	boundary condition for sigma		*/
...
void Initial_Profile(double *sigmavec, double *r, double DD){		/*	initial profile of sigma		*/
...
double time_step(double *rvec, double DD) {					/*	a diffúziós egyenlet megoldásához szükséges minimális időlépés	*/
...
void Initial_dPress(double *dpressvec, double *pressvec, double DD){		/*	initial profile of pressure		*/
...
void int_step(double prad, double *dpressvec, double *sigmavec, double *rvec, double DD, double step, double y, double *ynew) {
...
/*	Kezdeti profilok betoltese a megadott vektorokba	*/
	...
	Initial_dPress(dpressvec,pressvec,DD);
  	...
  	t = 0.0;
   	t_integration = 250000.0 * 2.0 * M_PI; 					/*	numerikus integralas idotartama		*/
...
/*	ez utan kiszamolhato az eredeti t_drift keplet, ezt iratom ki a file-ba evben (ezert osztok le 2PI-vel	*/
			...
			for (i = 0; i < PARTICLE_NUMBER; i++) {	
				...
				if (radius[i][0] > RMIN && radius[i][0] < RMAX) { 
					...
					y = radius[i][0];
		     			...
		     			int_step(particle_radius,dpressvec,sigmavec,rvec,DD,deltat,y,&y_out);
		...
		if(max >= RMIN) {							/*	Ha a legtavolabbi porreszke merete nagyobb RMIN-nel, akkor folytatodik az integralas, kulonben kilep	*/
  			...
  			Perem(sigmavec, DD);						/*	loading boundary condition in each timestep		*/
...
//					particle_radius = 5.0 / AU2CM;
			     		...
			     		int_step(particle_radius,dpressvec,sigmavec,rvec,DD,deltat,y,&y_out);	/*	Integralas	*/
