...
void eqrhs(double pradius, double sigma, double r, double *drdt) {
...
void int_step(double prad, double *sigmavec, double *rvec, double step, double y, double *ynew)
{
	...
	eqrhs(prad, sigma, y, &dy1);
	
	ytemp = y + 0.5 * step * dy1;
	eqrhs(prad, sigma, ytemp, &dy2);
		
	ytemp = y + 0.5 * step * dy2;
	eqrhs(prad, sigma, ytemp, &dy3);
	
	ytemp = y + step * dy3;
	eqrhs(prad, sigma, ytemp, &dy4);
...
int main() {
   	...
   	double distance, reprmass, y, y_out, t, t_integration, deltat, particle_radius, sigmavec[NGRID+2], rvec[NGRID+2];
	char dens_name[64];
		...
		for (i = 0; i < PARTICLE_NUMBER; i++) {
			...
			if (radius[i][0] > RMIN) { 
				y = radius[i][0];
		     		...
		     		int_step(particle_radius,sigmavec,rvec,deltat,y,&y_out);
		     		radius[i][0] = y_out;
			} else {
				radius[i][0] = 0.0;
			}
