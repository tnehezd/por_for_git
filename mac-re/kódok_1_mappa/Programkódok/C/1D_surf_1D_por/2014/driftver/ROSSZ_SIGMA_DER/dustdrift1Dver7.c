...
void Perem(double *sigmavec, double DD, double alphvisc) {					/*	boundary condition for sigma		*/
...
void Initial_Profile(double *sigmavec, double *r, double DD, double alphvisc){		/*	initial profile of sigma		*/
...
/*	Kezdeti profilok betoltese a megadott vektorokba	*/
	Initial_Profile(sigmavec,rvec,DD,alpha_visc);
...
/*	ez utan kiszamolhato az eredeti t_drift keplet, ezt iratom ki a file-ba evben (ezert osztok le 2PI-vel	*/
		...
		if(max >= RMIN) {							/*	Ha a legtavolabbi porreszke merete nagyobb RMIN-nel, akkor folytatodik az integralas, kulonben kilep	*/
  			...
  			Perem(sigmavec, DD, alpha_visc);				/*	loading boundary condition in each timestep		*/
