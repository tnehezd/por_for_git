...
double optev, optdr, optgr, opttwopop, optdze;		/*	parameterek, amelyekkel ki es be lehet kapcsolni a szamolasok egy reszet:
...
void interpol(double *invec, double *rvec, double pos, double *out, double rd) {
...
/*	Mivel a kulongozo parametereket csak a megadott gridcella pontokban ismerjuk, de ez nem feltetlen egyezik meg a reszecskek pozicijaval, ezert minden fontos parametert interpolalunk a reszecskek tavolsagara	*/
	interpol(sigmavec,rvec,y,&sigma,DD);
	interpol(dpressvec,rvec,y,&dpress,DD);
	interpol(ugvec,rvec,y,&ugas,DD);
...
void find_r_annulus(double *rvec, double rout, double *ind_oi, double *ind_oo) {
	...
	int i;
	double rmid, rtemp;
	double roimH = (rout - scale_height(rout)) - DD / 2.0;		/*	A nyomasi maximum az rout pontoban van, ettol rout - H - DD / 2 es rout + H -DD / 2 kozott van a korgyuru belso hatara (azert DD/2, hogy biztosan 1 cellat tudjunk kijelolni, ne pedig egy tartomanyt)	*/
	double roipH = (rout - scale_height(rout)) + DD / 2.0;		
	double roomH = (rout + scale_height(rout)) - DD / 2.0;		/*	Az alabbi ketto pedig a kulso hatarat adja meg a korgyurunek	*/
...
void Print_Mass(double step, double *rvec, double partmassind[][3], double partmassmicrind[][3], double t, double *dpressvec) {
...
/*	Ha van belso dze is, akkor itt menti el a kuslo es belso dze helyet	*/		
	...
	find_r_annulus(rvec,tav,&ind_oi,&ind_oo);		/*	A kuslo nyomasi maximum korul 2H tavolsagban keres korgyurut, a fuggveny visszaadja a cellak indexet	*/

	int index_i,index_i2,i;
	double mass = 0., mass2 = 0.;

/*	A cm-es porszemcsek tomegnovekedese	*/
	for (i = 0; i < PARTICLE_NUMBER; i++) {
...
/*	A partmassind tombben van eltarolva a reszecske indexe (ez az integralas soran allando, ezzel lehet abrazolni szinesben), az altala kepviselt reprezentativ tomeg (ez az integralas soran allando) es a grid cella indexe, amelyben eppen tartozkodik (ezt minden lepesben kiszamolja a program	*/
		index_i = partmassind[i][2];			/*	a gridcella indexe, amelyben a reszecske tartozkodik eppen	*/
		double masstemp = 0.;
...
/*	Ha a reszecse a kiszamolt korgyuruben tartozkodik, akkor a mass valtozohoz hozzaadjuk a tomeget --> igy tudjuk a tomegnovekedest kiszamolni	*/
		if ((index_i >= (int)ind_oi) && (index_i <= (int)ind_oo)) {
			...
			masstemp = partmassind[i][1];		/*	Az adott reszecske altal kepviselt reprezentativ tomeg	*/	
			mass = mass + masstemp;

		}

/*	A mikronos reszecskek tomegnovekedese	*/
/*	Hasonlo a fenti, cm-es porszemcsek altali tomegnovekedes meresehez	*/		
		if(twopop == 1.) {	/*	Ezt csak akkor szamolja, ha 2 populacios szimulacio fut eppen	*/
			index_i2 = partmassmicrind[i][2];
			double masstemp2 = 0.;

			if ((index_i2 >= (int)ind_oi) && (index_i2 <= (int)ind_oo)) {
	
				masstemp2 = partmassmicrind[i][1];
				mass2 = mass2 + masstemp2;

			}
		} else {		/*	Ha nem ketpoplacios a szimualcio, akkor a mikronos reszecskekbol szarmazo tomegnovekedes 0	*/
			mass2 = 0.;
		}
...
void Print_Sigmad(char *dust_name, char *dust_name2, double min, double radius[][2], double radiusmicr[][2], double *sigmad, double *sigmadm) {
	...
	for(i=0;i<PARTICLE_NUMBER;i++){
		...
		if (radius[i][0] >= RMIN) {			/*	a cm-es por feluletisurusege	*/
			fprintf(fil,"%lg  %lg \n",radius[i][0],sigmad[i]);
		} 

		if(opttwopop == 1) {				/*	itt irja ki a mikronos por feluletisuruseget	*/

			if (radiusmicr[i][0] >= RMIN) {
				fprintf(sid,"%lg  %lg \n",radiusmicr[i][0],sigmadm[i]);
			}
		}
	}
...
/*void sort(double rv[][2]) {

	double temp, temp2;
	int i, step;

	for(step = 1; step <= (2*PARTICLE_NUMBER-1); step++) {

		for(i = 0; i <= (2*PARTICLE_NUMBER-2); i++) {
...
void Get_Sigmad(double L, double max, double min, double rad[][2], double radmicr[][2], double partmass[][3], double partmassmic[][3], double *sigma_d, double *sigma_dm, double *massvec, double *massmicrvec) {
...
/*	a feluletisurusegek interpolalasa 2 populacio eseten	*/
	if(opttwopop == 1) {
		for(i=0; i < PARTICLE_NUMBER; i++) {
			...
			double ri1, ro1, ri2, ro2, dd1, dd2;
			double d,dm;
...
void Get_Radius(char *nev, int opt, double radius[][2], double *pressvec, double *dpressvec, double *sigmavec, double *sigmad, double *rvec, double *ugvec, double deltat, double t) {		/*	a bemenet dimenziótlan egységekben van!!!	*/
...
/*	itt az opt a 2 populacios modellre utal, ha az 1, akkor a mikronos reszecskekre szamol - ehelyett egyebkent valoszinu lehetne az opttwopop-ot is hasznalni	*/
			if(t == 0) {
				if(opt != 1) drdt[i] = (fabs(y_out - y)/(deltat));	/*	csak a cm-es esetben irja ki a drift idoskalajat	*/
			}
...
void Count_Mass(double radius[][2], double radiusmicr[][2], double partmassind[][3], double partmassmicrind[][3], double *massvec, double *massmicr) {
...
/*	mikronos porszemcsek!	*/
 		rmid = (radiusmicr[i][0] - RMIN) / DD;     						/* 	The integer part of this gives at which index is the body			*/
		rindex = (int) floor(rmid+0.5);							/* 	Ez az rmid egesz resze --> floor egeszreszre kerekit lefele, a +0.5-el elerheto, hogy .5 felett felfele, .5 alatt lefele kerekitsen						*/

		partmassmicrind[i][0] = (double)i;							/*	index of the particles				*/
		partmassmicrind[i][1] = massmicr[i];							/*	mass of the particles				*/
		partmassmicrind[i][2] = rindex;							/*	initial distance of the particles				*/
...
/*	A reszecskek adatait tartalmazo file sorainak szama (azaz a reszecskek szama) elmentve egy integerbe, amennyiben a futas soran szamol a program driftet	*/
	...
	char mv[1024], dens_name[1024],size_name[1024], porout[1024], poroutmicr[1024], massout[1024], dust_name[1024], dust_name2[1024];
   	double radius[PARTICLE_NUMBER][2],radiusmicr[PARTICLE_NUMBER][2],radius_rec[PARTICLE_NUMBER][2];
	double massvec[PARTICLE_NUMBER], massmicrvec[PARTICLE_NUMBER];
...
/*	tomegnovekedesi file letrehozasa az aktualis mappaba - ez lehet, hogy egy kulon opcio lesz a kimeneti adatok meretenek csokkentesere	*/
	...
	double partmassind[PARTICLE_NUMBER][3], partmassmicrind[PARTICLE_NUMBER][3];
	double L = 0.;
	double sigmad[PARTICLE_NUMBER];
	double sigmadm[PARTICLE_NUMBER];
...
/*	A minimum kereseshez letrehozza a cm-es reszecskek tavolsaganak reciprokat	*/
			...
			max = find_max(radius);					/*	Megkeresi, hogy melyik a legtavolabbi cm-es reszecske a kozponti csillagtol	*/
			min = find_max(radius_rec);				/*	Megkeresi a tavolsag reciprokanak maximumat, azaz a legkisebb tavolsagra levo cm-es reszecsket	*/
			min = 1. / min;
...
/*	ha 2 populacios a szimulacio, a fentihez hasonloan megkeresi a legnagyobb es a legkisebb tavolsagra levo mikronos reszecsket	*/
			if(twopop == 1) {
				...
				max2 = find_max(radiusmicr);					/*	Megkeresi, hogy melyik a legtavolabbi reszecske a kozponti csillagtol	*/
				min2 = find_max(radius_rec);
				min2 = 1. / min2;
...
/*	Az adatok kiirasahoz szukseges file-ok neveinek elmentese	*/
					...
					if(t==0) {
/*	A reszecskek tomeget tartalmazo tomb frissitese		*/
						Count_Mass(radius,radiusmicr,partmassind,partmassmicrind,massvec,massmicrvec);
/*	Ha van tomegnovekedes, akkor a por feluletisurusegenek kiszamolasa itt tortenik	*/
						if(optgr == 1.) {			
							Get_Sigmad(L,max,min,radius,radiusmicr,partmassind,partmassmicrind,sigmad,sigmadm,massvec,massmicrvec);
						}
					}
...
/*	A tomegnovekedesi file-ba az adatok kiirasa	*/
					Print_Mass(L,rvec,partmassind,partmassmicrind,t,dpressvec);

/*	Ha van pornovekedes, kiirja a por felultisuruseget egy file-ba --> a pornovekedeshez szukseges egyaltalan ezt kiszamolni!	*/
					if(optgr == 1.) {
						Print_Sigmad(dust_name,dust_name2,mint,radius,radiusmicr,sigmad,sigmadm);
					}
...
/*	Itt szamolja ki a program a cm-es reszecskek uj tavolsagat (es meretet, ha kell)	*/
				Get_Radius(nev,optsize,radius,pressvec,dpressvec,sigmavec,sigmad,rvec,ugvec,deltat,t);
 
/*	Ha a futas 2 populacios, akkor az optsize erteke 1	*/
/*	Itt szamolja ki a program a mikoronos reszecskek uj tavolsagat	*/
				if(opttwopop == 1.) {
					optsize = 1;
					Get_Radius(nev,optsize,radiusmicr,pressvec,dpressvec,sigmavec,sigmad,rvec,ugvec,deltat,t); 
				}
...
/*	Ha a parameterek beolvasasa sikertelenvolt, akkor a program kilep	*/	
	if (0 != retCode) {
		exit(retCode);
	}
...
/*	A korong parametereit beolvassa				*/
