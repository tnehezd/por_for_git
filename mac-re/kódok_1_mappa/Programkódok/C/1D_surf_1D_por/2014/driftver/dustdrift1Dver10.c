...
void int_step(double time, double prad, double *pressvec, double *dpressvec, double *sigmavec, double *rvec, double step, double y, double *ynew, double *pradnew) {
...
void Print_Mass(double *rvec, double partmassind[][3], double t, double *dpressvec) {
	...
	if(t==0) {
		...
		int dim = find_num_zero(rvec,dpressvec);
		double r_count[dim];
		double temp_new = 0.;
		double temp = 0.;
		double rout = r_dze_o;
...
void Get_Radius(char *nev, double radius[][2], double *pressvec, double *dpressvec, double *sigmavec, double *rvec, double deltat, double t) {
	...
	for (i = 0; i < PARTICLE_NUMBER; i++) {	
		...
		if (radius[i][0] > RMIN && radius[i][0] < RMAX) { 
			...
			y = radius[i][0];
     			...
     			int_step(t,particle_radius,pressvec,dpressvec,sigmavec,rvec,deltat,y,&y_out,&prad_new);
...
void Count_Mass(double radius[][2], double partmassind[][3], double *massvec, double *dpressvec, double *rvec, double t) {
	...
	Print_Mass(rvec,partmassind,t, dpressvec);
...
void Mk_Dir(char *nev) {
	...
	char comm1[1024], comm2[1024],comm3[1024],temp_nev1[1024],temp_nev2[1024],temp_nev3[1024];
	char comm[1024];
	int i, step, mappa1, mappa2, mappa3;
	step = 0;

	sprintf(temp_nev1,"ALPHA_%lg_R%lg",alpha_visc,SIGMAP_EXP);
	sprintf(temp_nev2,"./%s/RMIN%lg-RMAX%lg",temp_nev1,RMIN,RMAX);
	sprintf(temp_nev3,"%s/RDZEO_%lg_DRDZEO_%lg",temp_nev2,r_dze_o,Dr_dze_o);

	sprintf(comm1,"mkdir %s",temp_nev1);
	sprintf(comm2,"mkdir %s",temp_nev2);
	sprintf(comm3,"mkdir %s",temp_nev3);
...
/*	A ciklus ellenorzi, hogy letezik-e mar ez adott mappa, ha nem, letrehozza a megadott neven...	*/
	if (mappa1 == 0) {	/*	Ha nem letezik a temp_nev1 mappa, létrehozza és létrehozza az összes almappát is	*/
		mappa2=system(comm2);
		mappa3=system(comm3);				

		printf("... A kimeneti file-ok tarolasahoz az \"%s\" mappa elkeszult ...\n\n\n",temp_nev3);

		sprintf(nev,"%s",temp_nev3);				/*	A mappa nevenek eltarolasa azert, hogy a file-okat az eppen aktualis mappaba tudja kesobb kiirni a program	    */
...
/*	...ha letezik az output mappa, akkor egy do-while ciklussal szamozott mappat hoz letre (pl. output.0), es addig fut a ciklus, mig aztan nem talalja mar a soron kovetkezo szamozott mappat. Ekkor letre hozza azt	*/
	} else {	/*	ha létezik a temp_nev1 mappa, megnézi, hogy léteznek-e az almappák	*/

		mappa2=system(comm2);

		if(mappa2 == 0) {	/*	ha nem létezik a temp_nev2 mappa, létrehozza azt, és a benne levő almappát is	*/

			mappa3=system(comm3);
			sprintf(nev,"%s",temp_nev3);				/*	A mappa nevenek eltarolasa azert, hogy a file-okat az eppen aktualis mappaba tudja kesobb kiirni a program	    */							
		
		} else {		/*	ha létezik a temp_nev2 mappa, létrehozza a benne levő almappát 			*/
		...
		printf("... A kimeneti file-ok tarolasahoz az \"%s\" mappa letrehozasa meghiusult ...\n",temp_nev3);
		printf("... A mappa valoszinuleg mar letezik, uj mappa letrehozasa ...\n\n\n");
	
/*	Ha létezik a temp_nev1 mappa, létrehoz egy újat, ami mögé odaírja, hogy _out és egy számot	*/

		do{
			for(i=0;i<=step;i++){
				sprintf(nev,"%s_out%d",temp_nev1,i);			/*	A mappa nevenek eltarolasa azert, hogy a file-okat az eppen aktualis mappaba tudja kesobb kiirni a program	    */
				sprintf(comm1,"mkdir %s",nev);	
			}

			sprintf(temp_nev2,"./%s/RMIN%lg-RMAX%lg",nev,RMIN,RMAX);
			sprintf(temp_nev3,"%s/RDZEO_%lg_DRDZEO_%lg",temp_nev2,r_dze_o,Dr_dze_o);
			sprintf(comm2,"mkdir %s",temp_nev2);
			sprintf(comm3,"mkdir %s",temp_nev3);
...
int main() {
   	...
   	int linesout;
   	double t, t_integration, deltat, sigmavec[NGRID+2], rvec[NGRID+2], pressvec[NGRID+2], dpressvec[NGRID+2];
	double max;
	char dens_name[1024],size_name[1024], nev[1024], cmonth[1024], out[1024], porout[1024], massout[1024], mv[1024];
...
/*	Kezdeti profilok betoltese a megadott vektorokba	*/
	...
	timePar(&TMAX,&WO,&TCURR);
  	...
  	t = 0.0;
   	t_integration = TMAX * 2.0 * M_PI; 						/*	numerikus integralas idotartama		*/
	...
	Mk_Dir(nev);								/*	mappa letrehozasa az adatok letrehozasahoz	*/
   	...
   	do {
		...
		if(max >= RMIN) {						/*	Ha a legtavolabbi porreszke merete nagyobb RMIN-nel, akkor folytatodik az integralas, kulonben kilep	*/
			...
			if((fmod(time, (TMAX/WO)) < deltat || time == 0) && L-time < deltat){							
				...
				Count_Mass(radius,partmassind,massvec,dpressvec,rvec,t);
			...
			Get_Radius(nev,radius,pressvec,dpressvec,sigmavec,rvec,deltat,t); 
