...
double sigma_dust(double r) {
    ...
    if (r <= SNOWLINE) {
        return DUST2GASR*sigma_null()*pow(r,SIGMAP_EXP);
    }
    
    if (r > SNOWLINE) {
        double sigma0 = sigma_null()*ICEFACTOR;
...
int main() {  
  ...
  int i, index, NGRID; //ez egyben a reprezentativ reszecskek szamat is jelenti
  double r, sd, pressure, dpdr, DD, reval, reval2, reppmass;
  double f_drift, f_frag, u_frag, u_frag2, G2, H, v_kep, c_s, v_kep2, c_s2, rmid, rindex, coef1, coef2, coef3,
         Sigma, Sigma_cgs, P, dPdr, dlnPdlnr, rho_p, s_drift, s_frag, s_df;
  double s_max;
