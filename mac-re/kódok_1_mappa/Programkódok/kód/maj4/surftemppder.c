...
void pressure(double *temp, double *r, double *sigma, double *p) {

	int i;

	for(i = 1; i <= ngrid; i++) {
		p[i] = rho0(temp[i],r[i],sigma[i]) * asp_ratio * asp_ratio * 4.0 * M_PI / r[i];
	}
...
/* i 1 - n-1 és p(0) = p(1) és p(n) = p(n-1) */
...
void Initial_Profile(double *sigmavec, double *r, double *tempvec, double *pvec, double *dpvec){		/*	initial profile of sigma		*/
	...
	pressure(tempvec, r, sigmavec, pvec);
	pressure_der(pvec,dpvec);

	for(i = 1; i <= ngrid; i++) {
		fprintf(surfinitdat,"%lg   %lg  %lg  \n", r[i], sigmavec[i],dpvec[i]);
	}
...
Initial_Temp(double *tempvec, double *rvec){			/*	initial temperature profile	*/
...
int main(){
  	...
  	int i;
  	double time, sigmavec[ngrid+2], rvec[ngrid+2], tempvec[ngrid+2], pvec[ngrid+2], dpvec[ngrid+2];
...
//  	surftimedat = fopen("surface.dat","w");
	...
	Initial_Temp(tempvec,rvec);
  	Initial_Profile(sigmavec,rvec,tempvec,pvec,dpvec);
/*	pressure(tempvec,rvec,sigmavec,pvec);
	pressure_der(pvec,dpvec);
*/
...
/*	Animacio letrehozasa az elkeszult file-okbok. A szamozott file-okat egy for ciklussal nyitja meg a gnuplot.	*/		
			...
			f_dens=fopen(dens_name,"w");
  			...
  			for(i = 1; i <= ngrid; i++) {							
				fprintf(f_dens,"%lg %lg %lg %lg\n",rvec[i], sigmavec[i],tempvec[i],dpvec[i]);
			}
