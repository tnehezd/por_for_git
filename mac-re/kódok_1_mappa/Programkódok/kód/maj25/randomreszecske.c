...
#define	min	1.0
#define	max	100.0
#define pwind	-2.0			/*	distribution power			*/
#define N	20000			/*	number of random generated numbers	*/
#define chunk 	10.0
...
int main() {
	...
	double radius[N],xsort[N], count,size,temp,counttemp,countloc,xrange[(int)dr],xpos[(int)(N/2.0)],xneg[(int)(N/2.0)],x[N],y[N],z[N],vx[N],vy[N],vz[N],mass[N];
	double rpos[N], rneg[N], r[N], phi[N];
	int i,h;
	...
	count = 0.;
 	...
 	double locmin1, locmax1, locmin2, locmax2, locmin3, locmax3, locmin4, locmax4;	/*	min max ertekek a random hely- es sebessegvektor generalashoz	*/
	locmin1 = 1.0;	/*	csillagaszati egysegben, r-hez sqrt(x*x + y*y) koordinatakhoz	*/
	locmax1 = 100.0;
	locmax2 = -100.0;	/*	csillagaszati egysegben, r-hez sqrt(x*x + y*y) koordinatakhoz	*/
	locmin2 = -20.0;

	locmin3 = 0.0;
	locmax3 = 2.0*M_PI;
	locmin4 = -2.0;
	locmax4 = 2.0;		/*	csillagaszati egysegben a z koordinatakhoz	*/
	...
	for(i = 0; i < (int)(N/2.0); i ++) {

		rpos[i] = random_gen(locmin1,locmax1);
		rneg[i] = -random_gen(locmin1,locmax1);
	...
	for(i = 0; i < N; i++) {
		...
		phi[i] = random_gen(locmin2,locmax2);
		x[i] = r[i] * cos(phi[i]);
		y[i] = r[i] * sin(phi[i]);

//		printf("%lg %lg %lg %lg \n",r[i],cos(phi[i]),sin(phi[i]), sqrt(x[i] * x[i] + y[i] * y[i]));
