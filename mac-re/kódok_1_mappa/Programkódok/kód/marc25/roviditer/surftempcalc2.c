...
double calc_temp(double r, double localsigma, double localtemp) {
...
void Initial_Temp(double *tempvec, double *rvec, double localtemp){			/*	initial temperature profile	*/

	int i;
//	tempinitdat = fopen("tempinit.dat","w");

	tempvec[1] = calc_temp(rvec[1],sigmainit(rvec[1]),localtemp);

	for(i = 2; i <= ngrid; i++) {
		localtemp = tempvec[i-1];
		tempvec[i] = calc_temp(rvec[i],sigmainit(rvec[i]),localtemp);
...
int main(){
	...
	clock_t start = clock();
  	int i;
  	double time, sigmavec[ngrid+2], rvec[ngrid+2], tempvec[ngrid+2], tempin;
	...
	Initial_Temp(tempvec,rvec,tempin);
  	...
  	do {
  		...
  		for(i = 1; i <= ngrid; i++) {					/* 	solving d(sigma*nu)/dt = 3*nu*d2(sigma*nu)/dr2 + 9*hu/(2*r)*dsigma/dr	*/
				tempvec[i] = calc_temp(rvec[i],sigmavec[i],tempvec[i-1]);
