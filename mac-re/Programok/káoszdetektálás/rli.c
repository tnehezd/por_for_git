/*  KISZAMOLJA EGY ALTALUNK MEGADOTT PALYA mLCE-JET  */
...
int main(int argn,char *arg[]) {					/*  a parancssorbol keri be az adatokat  */
 	...
 	double K;
 	long double x,p,zero,w_1,w_2,w_old_1,w_old_2,ww_1,ww_2,ww_old_1,ww_old_2,wlength,wwlength,lce,pp,xx,llce,rli, Delta;
 	...
 	if(argn==1) printf("***   K   nmax   0<x<1   0<p<1   orbitfile  vecfile \n");	/*  K: nemlin param  */
		...
		llce=0.0;
   		...
   		for(n=1;n<=nmax;n++) {						/*  hanyszor iteraljuk az egyes palyakat  */
    			...
    			pp=fmod(pp-K*sin(xx+M_PI),2*M_PI);
    			xx=fmod(xx+pp+M_PI,2*M_PI);

   	 		w_1=w_old_1+K*cos(x)*w_old_2; 
    			w_2=w_old_1+w_old_2+K*cos(x)*w_old_2;
  	 		ww_1=ww_old_1+K*cos(xx)*ww_old_2; 
    			ww_2=ww_old_1+ww_old_2+K*cos(xx)*ww_old_2;

    			wlength=sqrt(w_1*w_1+w_2*w_2);  
    			wwlength=sqrt(ww_1*ww_1+ww_2*ww_2);  
   
    			lce=lce+log(wlength);
    			llce=llce+log(wwlength);
   
			rli=fabs(lce-llce)/n;

    			printf("lepes: %i, elteres vektor hossza: %Lg,   LCE: %Lg\n, rli: %Lg",n,wlength,lce/n, rli);
    			fprintf(fmap0,"%i %Lg\n",n,rli);
    			fflush(fmap0);
/*    getchar();	*/
    			...
    			w_old_1=w_1/wlength;
    			w_old_2=w_2/wlength;
    			ww_old_1=ww_1/wwlength;
    			ww_old_2=ww_2/wwlength;
 
    			if(x<zero) x=x+2*M_PI;
    			if(p<zero) p=p+2*M_PI;
    			if(x>2*M_PI) x=x-2*M_PI;
    			if(p>2*M_PI) p=p-2*M_PI;
    			if(xx<zero) xx=xx+2*M_PI;
    			if(pp<zero) pp=pp+2*M_PI;
    			if(xx>2*M_PI) xx=xx-2*M_PI;
    			if(pp>2*M_PI) pp=pp-2*M_PI;
